...
export default function Sidebar() {
  ...
  useEffect(() => {
    const handleReceiveMessage = (message) => {
      const token = cookies?.access_token;
      // Only update counts if the current user is the intended recipient
      if (token && message?.recipient === user.id && message?.sender !== user.id) {
        dispatch(fetchUnreadCounts({ userId: user.id, token }));
        // Bump sender to top by updating last message time
        dispatch(
          bumpLastMessageTime({
                ...
                sortedUsers.map((chatUser) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className={`ml-auto w-2 h-2 rounded-full ${chatUser?.isOnline == true
                            ? "bg-green-400"
                            : "bg-gray-300"
                          }`}
                      />
