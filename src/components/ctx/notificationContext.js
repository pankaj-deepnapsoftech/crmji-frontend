...
const NotificationContextProvider = ({ children }) => {
  ...
  const seenNotificationHandler = async () => {
    try {
      ...
      const data = await response.json();
      // Reset both general and chat unseen counts so badges clear immediately
      dispatchNotificationAction({ type: "RESET_UNSEEN_COUNT" });
      return dispatchNotificationAction({ type: "RESET_CHAT_UNSEEN_COUNT" });
    } catch (err) {
      toast.error(err.message);
    }
  };
  ...
  const notificationReducer = (state, action) => {
    ...
    } else if (action.type === "RESET_CHAT_UNSEEN_COUNT") {
      return {
        ...state,
        unseenchatNotifications: 0,
      };
