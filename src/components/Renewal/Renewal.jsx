  ...
  <div>
    ...
    {viewMode === "card" && (
      <div className="px-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {filteredData.map((item) => (
          <div
            key={item._id}
            className="bg-white shadow-lg rounded-xl border border-gray-200 p-5 hover:shadow-xl transition"
          >
            ...
            <div className="space-y-1 text-sm">
              <p><b>Contract Type:</b> {item.contractType}</p>
              <p><b>Contract No:</b> {item.contractNumber}</p>
              <p><b>Product:</b> {item.productName}</p>
              <p><b>Phone:</b> {item.phnNumber}</p>
              <p><b>Renewals:</b> {item.renewalTimes}</p>
    ...
    {viewMode === "table" && (
      <>
        ...
        <div className="w-[max-content] m-auto my-7">
          <button
            className="text-sm mt-2 bg-[#1640d6] py-1 px-4 text-white border-[1px] border-[#1640d6] rounded-3xl 
            disabled:bg-[#b2b2b2] disabled:border-[#b2b2b2] disabled:cursor-not-allowed md:text-lg"
            disabled={!canPreviousPage}
            onClick={previousPage}
          >
            Prev
