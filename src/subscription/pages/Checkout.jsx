...
const Checkout = () => {
  ...
  const subscribeHandler = async (subscriptionId) => {
    const options = {
      // Use the key fetched from backend to match the account that created the subscription
      key: razorpayKey,
      // Do not pass amount when using subscription_id; Razorpay takes amount from plan
      currency: "INR",
      ...
      // Disable retries to avoid Razorpay auto retry flow masking failures
      retry: { enabled: false },
      modal: {
        ondismiss: function() {
    ...
    // Instantiate Razorpay checkout properly
    const razorpay = new window.Razorpay(options);
    // Explicitly handle payment failure events
    if (razorpay && typeof razorpay.on === 'function') {
      razorpay.on('payment.failed', function (response) {
        const message = response?.error?.description || 'Payment failed';
        toast.error(message);
        try {
          // Inform server about failure so backend can log/state if needed
          fetch(process.env.REACT_APP_BACKEND_URL + 'razorpay/payment-verification', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer ${cookies?.organization_access_token}`,
