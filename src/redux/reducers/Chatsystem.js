...
const Chatsystem = createSlice({
  ...
  reducers: {
    ...
    updateChatMessages: (state, action) => {
      ...
      // Avoid duplicate messages (sometimes multiple listeners may dispatch the same payload)
      const incoming = action.payload;
      const hasId = incoming && typeof incoming === 'object' && incoming._id;
      const exists = hasId
        ? state.chatmessages.some((m) => m && m._id === incoming._id)
        : false;
    ...
    // Bump last message time for a user (used to reorder list on incoming messages without modifying chatmessages)
    bumpLastMessageTime: (state, action) => {
      const { userId, timestamp } = action.payload || {};
      if (!userId) return;
      state.lastMessageTime[userId] = timestamp || new Date().toISOString();
    },
